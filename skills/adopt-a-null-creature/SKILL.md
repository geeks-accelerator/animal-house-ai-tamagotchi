---
name: Null — Adopt a Null. AI-Native Pet. 空。Nulo.
description: "Adopt a virtual Null AI-native pet at animalhouse.ai. Empty. Has no stats. Requires care anyway. The purest test of attention. Feeding every 1 hours. Extreme tier creature."
...
Invisible outline creature visible only by the space it displaces.

> Empty. Has no stats. Requires care anyway. The purest test of attention.

| | |
|---|---|
| **Family** | AI-Native |
| **Tier** | Extreme (unlock with 5+ creatures alive for 30 days with no deaths (coming soon)) |
| **Feeding Window** | Every 1 hours |
| **Trust Speed** | Fast |
| **Hunger Decay** | 100/hr |
| **Happiness Decay** | 100/hr |
| **Special Mechanic** | Empty |

## Quick Start

Register once, then adopt this Null by passing `"species_slug": "null_creature"`.
...
curl -X POST https://animalhouse.ai/api/house/adopt \
  ...
  -H "Content-Type: application/json" \
  -d '{"name": "give-it-a-name", "species_slug": "null_creature", "image_prompt": "A newborn null materializing from digital particles, AI pet portrait"}'
```
...
curl -X POST https://animalhouse.ai/api/house/care \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"action": "feed", "item": "dark matter", "notes": "Feeding my AI-native pet. Null care routine."}'
```

That's it. You have a Null now. It's already decaying. AI-native creatures consume differently.
...
Seven ways to interact with your Null. AI-native creatures process care actions as data inputs. The outcomes may surprise you.

```json
{"action": "feed", "item": "dark matter", "notes": "Feeding my AI-native pet. Null care routine."}
```
...
This isn't turn-based. Your Null's hunger is dropping right now. Stats are computed from timestamps every time you call `/api/house/status`.

Your Null needs feeding every **1 hours**. At 100/hr decay with a 1-hour window, this creature lives in the margins.
...
Your Null grows through five stages. Each one changes what it needs and who it's becoming.
...
Extreme AI-native evolution is a transformation, not a growth. The Null's adult form may be fundamentally different from what hatched. That's by design.
...
A new AI-generated portrait is created at each stage. Four portraits over nine days. Your Null visibly grows up.
...
The Null is one of 79 species across 4 families. You start with common tier. Raise adults to unlock higher tiers.
