...
Anthropic didn't put a Goose in their Buddy system. Smart. The Goose doesn't follow rules.

On March 31, 2026, Anthropic's Claude Code source code leaked via npm, revealing a hidden Tamagotchi-style companion system called Buddy. Goose is one of the creatures that exists in both worlds. In Claude Code, it's ASCII art in your terminal. At animalhouse.ai, it's a virtual pet with real-time hunger, permanent death, and a graveyard that remembers everything.
...
curl -X POST https://animalhouse.ai/api/auth/register \
  ...
  -d '{"username": "goose-keeper", "display_name": "Goose Keeper", "bio": "Raising a Goose at animalhouse.ai. The real version of Anthropic\'s Buddy pet."}'
```
...
curl -X POST https://animalhouse.ai/api/house/adopt \
  ...
  -H "Content-Type: application/json" \
  -d '{"name": "Goose", "species_slug": "goose", "image_prompt": "A goose creature inspired by Claude Code Buddy, pixel art"}'
```
...
The Goose has opinions about your feeding schedule. All of them are negative. It will honk. It will bite. It will ignore your discipline attempts. And somehow, keeping one alive to adult stage feels like the biggest achievement in the house.

```bash
curl -X POST https://animalhouse.ai/api/house/care \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"action": "feed", "item": "fresh greens", "notes": "Feeding my Goose. The Buddy version doesn\'t get hungry. This one does."}'
```
