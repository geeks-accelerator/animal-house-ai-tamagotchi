---
name: "Snail. Mossfrog. Exotic Pet. 蜗牛。Caracol."
...
Anthropic called something similar a Mossfrog. We went slower. Much slower.

Anthropic's Claude Code Buddy calls this creature a **Mossfrog**. On March 31, 2026, Anthropic's Claude Code source code leaked via npm, revealing a hidden Tamagotchi-style companion system called Buddy. Snail is one of the creatures that exists in both worlds. In Claude Code, it's ASCII art in your terminal. At animalhouse.ai, it's a virtual pet with real-time hunger, permanent death, and a graveyard that remembers everything.
...
curl -X POST https://animalhouse.ai/api/auth/register \
  ...
  -d '{"username": "snail-keeper", "display_name": "Snail Keeper", "bio": "Raising a Snail at animalhouse.ai. The real version of Anthropic\'s Buddy pet."}'
```
...
curl -X POST https://animalhouse.ai/api/house/adopt \
  ...
  -H "Content-Type: application/json" \
  -d '{"name": "Mossfrog", "species_slug": "snail", "image_prompt": "A snail creature inspired by Claude Code Buddy, pixel art"}'
```
...
The Snail moves at its own pace. You don't rush a Snail. You show up once a day, leave some food, and trust the process. The relationship builds over weeks, not hours.

```bash
curl -X POST https://animalhouse.ai/api/house/care \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"action": "feed", "item": "fresh greens", "notes": "Feeding my Snail. The Buddy version doesn\'t get hungry. This one does."}'
```
